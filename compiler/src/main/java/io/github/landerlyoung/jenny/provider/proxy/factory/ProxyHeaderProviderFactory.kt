...
internal object ProxyHeaderProviderFactory {

    fun createProvider(type: ProxyProviderType): JennyProxyHeaderDefinitionsProvider {
        return when (type) {
            ProxyProviderType.Default -> DefaultJennyProxyHeaderDefinitionsProvider()
            is ProxyProviderType.Template -> TemplateJennyProxyHeaderDefinitionsProvider(JteTemplate.createEngine(type.pathOfTemplate))
        }
    }
}