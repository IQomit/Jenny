...
internal object ProxySourceProviderFactory {
    fun createProvider(type: ProxyProviderType): JennyProxySourceDefinitionsProvider {
        return when (type) {
            ProxyProviderType.Default -> DefaultJennyProxySourceDefinitionsProvider()
            is ProxyProviderType.Template -> TemplateJennyProxySourceDefinitionsProvider(JteTemplate.createEngine(type.pathOfTemplate))
        }
    }
}